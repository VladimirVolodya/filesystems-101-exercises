...
int64_t send_file(int img, int out, struct ext2_super_block* p_sb,
  ...
  for (size_t i = 0; i < EXT2_NDIR_BLOCKS; ++i) {
    if ((ret = send_blk(img, out, inode.i_block[i], blk_sz, left_read)) < 0) {
      return ret;
    }
    if ((left_read -= ret) <= 0) {
      return 0;
  ...
  uint32_t* blk_idxs = malloc(blk_sz);
  if (pread(img, blk_idxs, blk_sz, blk_sz * inode.i_block[EXT2_IND_BLOCK]) ==
      -1) {
    return -errno;
  }
  if (ret < 0) {
  ...
  if (pread(img, blk_didxs, blk_sz, inode.i_block[EXT2_DIND_BLOCK] * blk_sz) ==
      -1) {
    free(blk_idxs);
    free(blk_didxs);
    return -errno;
